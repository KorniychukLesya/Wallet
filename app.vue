<template class="h-screen">
  <div class="relative container mx-auto w-[40rem]  rounded bg-[#0C223C] font-sans">
    <header class=" flex justify-between pl-5 pb-3.5 pt-3.5 bg-[#2D4D70] bg-gradient-to-r from-[#0C223C]" >
      <h1 class="text-lg tracking-wider font-semibold text-sky-500">Wallet</h1>
        <div  class="absolute top-3 right-5 flex w-48 h-[32] flex-col items-center">
          <button class="flex justify-center items-center w-full bg-cyan-700  p-1.5 rounded-t-md " @click="changeVisibleSettings()">
          ...
          <button v-if="select.typesValuta.visibleSettings" class="flex w-full p-2.5 h-[84px] bg-sky-900 rounded-b-md" >
            <div  @click="changeFilterStatus()" 
                  class="flex items-center ">
              <img src="~/assets/images/filter.svg" alt="dollar" class="inline-block mr-1.5">
              <p class="bg-sky-900 text-xs text-white font-normal hover:text-sky-500">Show only Fiat</p>
            </div>        
          </button>
        </div>
    </header>
    <div class="px-5 pb-3.5 pt-1">
      <ul v-if="select.typesValuta.checedFilter">
        <li v-for="(item, index) in curentWalletFiat" 
            class=" pt-2.5 pb-3.5 flex justify-between items-center border-b  border-dashed border-[#24A0F5]">
          <div>
            <div class="flex">
              <div class="mr-1.5 mb-2.5 aspect-square rounded-sm w-7 h-7 bg-[#2D4D70] flex place-items-center justify-center " >
              <div class="flex place-items-center justify-center bg-cyan-400 aspect-square w-5 rounded-full shadow-[0_0_10px_2px_rgba(42,243,243,0.55)]">
            ...
            <div>
              <button class="w-32 h-7 mr-2.5 text-xs font-bold bg-sky-900 text-sky-500 uppercase rounded-sm">withdraw</button>
              <button class="w-32 h-7 text-xs font-bold bg-[#24A0F5] text-[#0D1C47] uppercase shadow-lg shadow-[0_0_10px_2px_rgba(0,133,255,0.55)] rounded-sm">deposit</button>
            </div>
          </div>
          <div class="flex flex-col items-end">
            <p class="mb-2.5 text-sm text-white font-normal leading-4">В заявках: {{store.round( item.total, index) }} {{ index }} </p>
            <p class="mb-2.5 text-sm text-white font-normal leading-4">Всего: {{store.round( item.reserved, index) }} {{ index }} </p>
      ...
      <footer class="flex flex-col p-2.5 mt-3.5 items-center  bg-sky-900 ">
        <div class="flex w-full justify-center">
          <img src="~/assets/images/Group.svg" alt="croup" class="inline-block mr-1.5">
          <div class="text-lg text-sky-500">{{store.totalBalans(evetSelect)}}</div>
          <select class="bg-sky-900 text-lg text-sky-500 font-normal outline-none" 
            v-model="select.typesValuta.selectedCurret" 
            @change="changeValuta($event)">
